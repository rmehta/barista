...
class Installer:
    ...
    def _init_bench(self, bench_dir: Path) -> None:
        ...
        if sys.platform == "linux":
            subprocess.run(["chown", "1000:1000", str(bench_dir)], check=False)
    ...
    def _bench_init_cmd() -> str:
        ...
        return (
            ...
            r"sed -i 's|/tmp/b/env|" + BENCH_RUNTIME_PATH + "/env|g' {} + && "
            "sed -i 's|/tmp/b/env|" + BENCH_RUNTIME_PATH + "/env|g' /work/env/pyvenv.cfg"
        )
    ...
    # ----- the control-plane container, in two flavours --------------------

    def _run_control_plane(self, bench_dir: Path) -> None:
        """Initial keep-alive container (tail -f /dev/null)."""
        argv = self._docker_run_args(bench_dir) + [
    ...
    def _docker_run_args(self, bench_dir: Path) -> list[str]:
        """Common `docker run` args for both control-plane variants."""
        env = parse_env_file(self.cfg.env_file)
        ...
        argv = [
            ...
            "-v", f"{bench_dir}:{BENCH_RUNTIME_PATH}",
        ...
        for label in self._control_plane_labels(env):
            argv += ["--label", label]
        return argv
    ...
    # ----- bench setup, executed inside the keep-alive container -----------
    ...
    def _new_site(self) -> None:
        ...
        self.docker.run([
            "docker", "exec", "-u", "frappe", "barista-bench-default",
            "bash", "-lc",
            f"cd {BENCH_RUNTIME_PATH} && bench new-site "
            "--no-mariadb-socket "
    ...
    def _wait_for_bench_http(self) -> None:
